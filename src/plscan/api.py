...
def most_persistent_clusters(
    ...
    Returns
    -------
    selected_clusters : np.ndarray[Any, np.dtype[np.int64]]
    ...
    return np.flatnonzero(
        (leaf_tree.min_size <= best_birth) & (leaf_tree.max_size > best_birth)
    )
...
def clusters_from_spanning_tree(
    ...
    *,
    ...
    sorted_mst : SpanningTuple
        A sorted minimum spanning tree.
    ...
    sample_weights : np.ndarray[Any, np.dtype[np.float32]], optional
